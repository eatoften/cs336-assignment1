...
def run_linear(
    ...
    dtype = weights.dtype
    layer = Linear(d_in, d_out,device,dtype)
...
def run_swiglu(
    ...
    layer.load_state_dict({"w1.weight":w1_weight,
                           "w2.weight":w2_weight,
                           "w3.weight":w3_weight})
    return layer(in_features)
...
def run_transformer_block(
    ...
    """
    device = weights.get("attn.q_proj.weight").device
    dtype = weights.get("attn.q_proj.weight").dtype
    layer = TransformerBlock(d_model, num_heads, d_ff, theta=theta, max_seq_len=max_seq_len, device=device, dtype=dtype)
...
def run_transformer_lm(
    ...
    """
    device = weights.get("token_embeddings.weight").device
    dtype = weights.get("token_embeddings.weight").dtype
    layer = TransformerLM(vocab_size,
            context_length,
            d_model,
            num_layers,
            num_heads,
...
def run_rmsnorm(
    ...
    device = weights.device
    dtype = weights.dtype
    layer = RMSNorm(d_model,eps,device,dtype)
    layer.load_state_dict({"weight":weights})
    return layer(in_features)
